...
def misc_measures(confusion_matrix):
    ...
    for i in range(1, confusion_matrix.shape[0]):
        ...
        _denominator = np.sqrt((cm1[0,0]+cm1[0,1])*(cm1[0,0]+cm1[1,0])*(cm1[1,1]+cm1[1,0])*(cm1[1,1]+cm1[0,1]))
        if _denominator == 0:
            mcc = 0
        else:
            mcc = (cm1[0,0]*cm1[1,1]-cm1[0,1]*cm1[1,0]) / _denominator
...
def evaluate(data_loader, model, device, task, epoch, mode, num_class):
    ...
    if mode == 'test':
        ...
        # # add title and labels font size
...
# for multiprocessing savefig
def plot_confusion_matrix(true_label_decode_list, prediction_decode_list, task):
    cm = ConfusionMatrix(actual_vector=true_label_decode_list, predict_vector=prediction_decode_list)
    fig,  ax = plt.figure(figsize=(16, 16))
    cm.plot(cmap=plt.cm.Blues,number_label=True,normalized=True,plot_lib="matplotlib", ax=ax)
    ...
    img.save(task+'confusion_matrix_test.jpg')
    plt.close(fig)
